...
/**
 * Init the ModbusRTU object.
 *
 * @param unitID the ModbusRTU slave id.
 * @param ctrlPin the digital out pin for RS485 read/write control.
 */
ModbusRTU::ModbusRTU(uint8_t _unitID, int _ctrlPin)
:ModbusRTU(Serial, _unitID, _ctrlPin)
{}

/**
 * Init the ModbusRTU object.
 *
 * @param serial the serial port used for the ModbusRTU communication
 * @param unitID the ModbusRTU slave id.
 * @param ctrlPin the digital out pin for RS485 read/write control.
 */
ModbusRTU::ModbusRTU(Stream &_serial, uint8_t _unitID, int _ctrlPin)
:serial(_serial)
{
    // set ModbusRTU slave unit id
    unitID = _unitID;
 ...
 */
void ModbusRTU::begin(unsigned long baud) {
	
	log_e("ModbusRTU::begin() start");
 ...
 */
uint16_t ModbusRTU::calcCRC(uint8_t *buf, int length) {
    int i, j;
    uint16_t crc = 0xFFFF;
    uint16_t tmp;
...
int ModbusRTU::poll() {
	...
	//log_e("available_len : %d", available_len);
    if ( available_len > 0 ) {
        // old data is too old
		...
		//log_e("lengthIn : %d",lengthIn);
    }
    else {
		//log_e("available_len <= 0");
        return 0;
    }

    //HexDump(Serial, bufIn, lengthIn); 
    //log_e("bufIn[0] :%d, unitID : %d",bufIn[0],unitID);
    // check unit-id
    if (bufIn[0] != unitID) {
		//log_e("unitID NOT match");
		return 0;
	}else{
		//log_e("unitID match");
	}
    ...
    //log_e("fc: %d, address: %d, length: %d",fc,address,length);
    ...
    if (calcCRC(bufIn, lengthIn - 2) != crc) {
        // standard ModbusRTU doesn't respond in case of
        // crc error
        return 0;
    }
    ...
    switch (fc) {
        ...
        case FC_READ_DISCRETE_INPUT: // read input state (digital in)
            ...
            if (cbVector[cb_index]) {
                cbVector[cb_index](fc, address, length);
            } else {
        ...
        case FC_WRITE_COIL: // write one coil (digital out)
            ...
            if (cbVector[CB_WRITE_COILS]) {
                cbVector[CB_WRITE_COILS](fc, address, 1);
            } else {
                cb_status = STATUS_ILLEGAL_FUNCTION;
            }
        ...
        case FC_WRITE_REGISTER:
            ...
            if (cbVector[CB_WRITE_HOLDING_REGISTERS]) {
                cbVector[CB_WRITE_HOLDING_REGISTERS](fc, address, 1);
            } else {
                cb_status = STATUS_ILLEGAL_FUNCTION;
            }
        ...
        case FC_WRITE_MULTIPLE_COILS: // write coils (digital out)
            ...
            // if we have uset callback.
            if (cbVector[CB_WRITE_COILS]) {
                cbVector[CB_WRITE_COILS](fc, address, length);
            } else {
                cb_status = STATUS_ILLEGAL_FUNCTION;
            }
 ...
 */
int ModbusRTU::readCoilFromBuffer(int offset) {
 ...
 */
uint16_t ModbusRTU::readRegisterFromBuffer(int offset) {
 ...
 */
void ModbusRTU::writeCoilToBuffer(int offset, int state) {
 ...
 */
void ModbusRTU::writeRegisterToBuffer(int offset, uint16_t value) {
 ...
 */
uint8_t ModbusRTU::writeStringToBuffer(int offset, uint8_t *str, uint8_t length) {
    int address = 3 + offset * 2;
